...
const Crud = () => {
    ...
    <div className="table">
      ...
      <div>
        <form onSubmit={hendleSubmit}>
          {lists.map((cur) => {
            return updateState === cur.id ? (
              <EditList cur={cur} lists={lists} setLists={setLists} />
            ) : (
              <div className="tre">
                <p>{cur.section} </p>
                <p>{cur.heading} </p>
                <p>{cur.task} </p>
                <td className="">
                  <button className="editet" onClick={() => hendleEdit(cur.id)}>
                    Edit
                  </button>
                  <button
                    className="deletet"
                    type="button"
                    onClick={() => hendleDelete(cur.id)}
                  >
                    Delete
                  </button>
                </td>
  ...
  function hendleSubmit(event) {
    ...
    const newlist = lists.map((li) => {
      return li.id === updateState
        ? { ...li, section: section, heading: heading, task: task }
        : li;
    });
    setLists(newlist);
    setUpdateState(-1);
...
function EditList({ cur, lists, setLists }) {
  const handInputhead = (event) => {
    ...
    const newlist = lists.map((li) => {
      return li.id === cur.id ? { ...li, section: value } : li;
    });
    setLists(newlist);
  };
  const handInputheading = (event) => {
    const value = event.target.value;
    const newlist = lists.map((li) => {
      return li.id === cur.id ? { ...li, heading: value } : li;
    });
    setLists(newlist);
  };
  const handInputtask = (event) => {
    const value = event.target.value;
    const newlist = lists.map((li) => {
      return li.id === cur.id ? { ...li, task: value } : li;
    });
    setLists(newlist);
  };
